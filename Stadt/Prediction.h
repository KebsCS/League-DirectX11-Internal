...
public:
	...
	static Vector3 PosAfterTime(GameObject* obj, const float& delay)
	{
		...
		float distance = (ms * delay) - obj->GetBoundingRadius();
	...
	static Vector3 Preidct33(GameObject* local, GameObject* target, float range, float radius, float castTime, float speed, int collision)
	{
		...
		if (speed >= 9999.f)
		{
			return PosAfterTime(target, castTime);
		}
		...
		Vector3 predictedPos = PosAfterTime(target, travelTime);
		...
		while (std::abs(travelTime - missileTime) > 0.01f) // 10ms tolerance
		{
			...
			predictedPos = PosAfterTime(target, travelTime);
