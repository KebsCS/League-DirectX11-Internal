...
static constexpr auto oLocalPlayer = 0x30F990C;// 0x310990C; // A1 ? ? ? ? 8B 54 24 28 85 C0
static constexpr auto oGameTime = 0x30F08CC;// 0x3101384; //  0F 57 C0 F3 0F 5C 0D ? ? ? ? 0F 2F C1
...
static constexpr auto oHeroList = 0x185a058;// 0x186abf4;
static constexpr auto oMinionList = 0x185c5dc;// 0x186cdb8;

static constexpr auto oChatClientPtr = 0x30ebbf0;// 0x3109E28;// 0x30fc790;
static constexpr auto oSendChat = 0x5F49B0;// 0x5F42A0; // outdated
//static constexpr auto oPrintChat = 0x589770;
static constexpr auto oIsNotWall = 0x99EFD0;// 0x9ABB50;// 0x9A9570; // [actual address in first opcode] E8 ? ? ? ? 8B 0F 0F 57 C0
static constexpr auto oIsBrush = 0x99F130;// 0x9ABBB0;// 0x9A96D0; // [actual address in first opcode] E8 ? ? ? ? 8A D8 8D 44 24 18
static constexpr auto oObjectManager = 0x185C544;// 0x186CD20; // FF 74 24 2C 8B 0D ? ? ? ? E8 ? ? ? ? 8B F0

static constexpr auto oViewMatrix = 0x3122690; // 0x31322A8; // 75 38 68 ? ? ? ? 8D 84 24 ? ? ? ?
static constexpr auto oRenderer = 0x3125530;// 0x3135148;
static constexpr auto oRendererWidth = 0xC;
static constexpr auto oRendererHeight = oRendererWidth + 0x4;

static constexpr auto oHudInstance = 0x185c57c;// 0x186cd58L;
...
static constexpr auto oSmoothPath = 0x1A4340;
static constexpr auto oBuildNavigationPath = 0x996850;
