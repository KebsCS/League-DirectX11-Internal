	...
	if (text_end - text > 2000 && !wrap_enabled)
	{
		...
		if (!g.LogEnabled)
		{
			...
			if (lines_skippable > 0)
			{
				...
				{
					const char* line_end = (const char*)crt::memchr(line, '\n', text_end - line);
					if (!line_end)
						line_end = text_end;
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
	...
	crt::memcpy(&data_backup, p_data, type_info->Size);
	...
	return crt::memcmp(&data_backup, p_data, type_info->Size) != 0;
}
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
	...
	else if (data_type == ImGuiDataType_S32 && crt::strcmp(format, "%d") != 0) // (FIXME-LEGACY: Patch old "%.0f" format string to use "%d", read function more details.)
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
	...
	if (TempInputText(bb, id, label, data_buf, IM_ARRAYSIZE(data_buf), flags))
	{
		...
		ImGuiDataTypeTempStorage data_backup;
		crt::memcpy(&data_backup, p_data, data_type_size);
		...
		value_changed = crt::memcmp(&data_backup, p_data, data_type_size) != 0;
		if (value_changed)
			MarkItemEdited(id);
	}
	return value_changed;
...
#define STB_TEXTEDIT_MOVEWORDRIGHT  STB_TEXTEDIT_MOVEWORDRIGHT_IMPL
	...
	static bool STB_TEXTEDIT_INSERTCHARS(ImGuiInputTextState* obj, int pos, const ImWchar* new_text, int new_text_len)
	{
		...
		if (pos != text_len)
			crt::memmove(text + pos + new_text_len, text + pos, (size_t)(text_len - pos) * sizeof(ImWchar));
		crt::memcpy(text + pos, new_text, (size_t)new_text_len * sizeof(ImWchar));
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text, const char* new_text_end)
{
	...
	const int new_text_len = new_text_end ? (int)(new_text_end - new_text) : (int)crt::strlen(new_text);
	...
	if (BufTextLen != pos)
		crt::memmove(Buf + pos + new_text_len, Buf + pos, (size_t)(BufTextLen - pos));
	crt::memcpy(Buf + pos, new_text, (size_t)new_text_len * sizeof(char));
...
static bool InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data, ImGuiInputSource input_source)
{
	...
	if (flags & ImGuiInputTextFlags_CallbackCharFilter)
	{
		...
		crt::memset(&callback_data, 0, sizeof(ImGuiInputTextCallbackData));
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
	...
	if ((init_state && g.ActiveId != id) || init_changed_specs)
	{
		...
		const int buf_len = (int)crt::strlen(buf);
		state->InitialTextA.resize(buf_len + 1);    // UTF-8. we use +1 to make sure that .Data is always pointing to at least an empty string.
		crt::memcpy(state->InitialTextA.Data, buf, buf_len + 1);
	...
	if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
	{
		...
		else if (is_paste)
		{
			if (const char* clipboard = GetClipboardText())
			{
				...
				const int clipboard_len = (int)crt::strlen(clipboard);
	...
	if (g.ActiveId == id)
	{
		...
		if (cancel_edit)
		{
			...
			if (!is_readonly && crt::strcmp(buf, state->InitialTextA.Data) != 0)
			{
		...
		if (apply_edit_back_to_user_buffer)
		{
			...
			if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackEdit | ImGuiInputTextFlags_CallbackAlways)) != 0)
			{
				...
				if (event_flag)
				{
					...
					if (buf_dirty)
					{
						IM_ASSERT(callback_data.BufTextLen == (int)crt::strlen(callback_data.Buf)); // You need to maintain BufTextLen if you change the text!
			...
			if (!is_readonly && crt::strcmp(state->TextA.Data, buf) != 0)
			{
	...
	{
		buf_display = hint;
		buf_display_end = hint + crt::strlen(hint);
	}
	...
	else
	{
		...
		if (is_multiline)
			...
			buf_display_end = buf_display + crt::strlen(buf_display);
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
	...
	if ((g.LastItemData.StatusFlags & ImGuiItemStatusFlags_HoveredRect) && !(flags & ImGuiColorEditFlags_NoDragDrop) && BeginDragDropTarget())
	{
		...
		{
			crt::memcpy((float*)col, payload->Data, sizeof(float) * 3); // Preserve alpha if any //-V512
			value_changed = accepted_drag_drop = true;
		}
		if (const ImGuiPayload* payload = AcceptDragDropPayload(IMGUI_PAYLOAD_TYPE_COLOR_4F))
		{
			crt::memcpy((float*)col, payload->Data, sizeof(float) * components);
			value_changed = accepted_drag_drop = true;
		}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
	...
	crt::memcpy(backup_initial_col, col, components * sizeof(float));
	...
	if (!(flags & ImGuiColorEditFlags_NoSidePreview))
	{
		...
		if (ref_col != NULL)
		{
			...
			{
				crt::memcpy(col, ref_col, components * sizeof(float));
				value_changed = true;
			}
		}
		PopItemFlag();
	...
	if (value_changed && crt::memcmp(backup_initial_col, col, components * sizeof(float)) == 0)
		value_changed = false;
	if (value_changed)
		MarkItemEdited(g.LastItemData.ID);
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
	...
	if (allow_opt_picker)
	{
		...
		for (int picker_type = 0; picker_type < 2; picker_type++)
		{
			...
			crt::memcpy(&previewing_ref_col, ref_col, sizeof(float) * ((picker_flags & ImGuiColorEditFlags_NoAlpha) ? 3 : 4));
...
void ImGuiMenuColumns::Update(float spacing, bool window_reappearing)
{
	if (window_reappearing)
		crt::memset(Widths, 0, sizeof(Widths));
...
struct ImGuiTabBarSection
{
	...
	ImGuiTabBarSection() { crt::memset(this, 0, sizeof(*this)); }
};
...
bool ImGui::TabBarProcessReorder(ImGuiTabBar* tab_bar)
{
	...
	crt::memmove(dst_tab, src_tab, move_count * sizeof(ImGuiTabItem));
	*tab2 = item_tmp;
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
	...
	tab_bar->TabsNames.append(label, label + crt::strlen(label) + 1);
