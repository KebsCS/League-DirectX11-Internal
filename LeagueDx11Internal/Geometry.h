...
namespace Geometry
{
	...
	public:
		...
		Polygon ToPolygon(int offset = 0)
		{
			...
			float outRadius = static_cast<float>(offset + this->Radius + this->RingRadius) / static_cast<float>(crt::cosf(2 * static_cast<float>(M_PI) / CircleLineSegmentN));
			...
			for (int i = 0; i <= CircleLineSegmentN; i++)
			{
				...
				auto point = Vector3(
					this->Center.x - outRadius * crt::cosf(angle),
					this->Center.y, this->Center.z - outRadius * crt::sinf(angle));
			...
			for (int i = 0; i <= CircleLineSegmentN; i++)
			{
				float angle = i * 2 * M_PI / CircleLineSegmentN;
				auto point = Vector3(
					this->Center.x - innerRadius * crt::cosf(angle),
					this->Center.y, this->Center.z - innerRadius * crt::sinf(angle));
